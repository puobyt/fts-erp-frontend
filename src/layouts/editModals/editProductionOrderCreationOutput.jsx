...
export default function EditProductionOrderCreationOutputForm({
  ...
  const formattedDate = productionOrderOutputData.productionCompletionDate
    ? new Date(productionOrderOutputData.productionCompletionDate).toISOString().split('T')[0]
    : '';
  const [formData, setFormData] = useState({
    ...
    unit: productionOrderOutputData?.unit || '',
    ...
    packingMaterials:
      productionOrderOutputData.packingMaterials &&
    ...
    try {
      await axiosInstance
        ...
        .catch((err) => {
          ...
          console.error('Error occured in editing production order in client side', err.message);
        });
    } catch (err) {
      console.error('Error occured in editing production orderin client side', err.message);
    }
  };
