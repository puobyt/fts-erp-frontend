...
export function TracebilityView() {
  ...
  // New state for batch number filter
  const [batchNumberFilter, setBatchNumberFilter] = useState(null);
  const [batchNumbers, setBatchNumbers] = useState([]); // Options for batch number dropdown
  ...
  // Update dataFiltered to include batch number filter
  const dataFiltered = applyFilter({
    inputData: materialData.filter(item => !batchNumberFilter || item.batchNumber === batchNumberFilter),
      ...
      >
        ...
        <Collapse in={isTableVisible}>
          ...
          <Autocomplete
            ...
            sx={{ width: 300, margin: '10px 0 0 16px' }}
